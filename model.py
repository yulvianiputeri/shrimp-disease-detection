...
def train_knn_optimized(X_train, y_train, X_test, y_test):
    ...
    knn_calib.fit(X_train, y_train)  # ← FIX: TAMBAHKAN .fit()
...
def train_svm_optimized(X_train, y_train, X_test, y_test):
    ...
    svm_calib.fit(X_train, y_train)  # ← FIX: TAMBAHKAN .fit()
...
def train_voting_ensemble(X_train, y_train, X_test, y_test, knn_model, svm_model):
    """
    Melatih Voting Ensemble dari model KNN dan SVM yang sudah dioptimasi.
    Menggunakan soft voting (probability-based) dengan equal weights.
    Mengembalikan akurasi, laporan klasifikasi, prediksi, label sebenarnya, dan model ensemble.
    """
    # Buat Voting Classifier dengan soft voting (equal weights)
    voting_clf = VotingClassifier(
        estimators=[
            ('knn', knn_model),
            ('svm', svm_model)
        ],
        voting='soft',  # Menggunakan probability untuk voting
        weights=None    # Equal weights (1:1)
    )
    ...
    return acc, report, preds, y_test, voting_clf
